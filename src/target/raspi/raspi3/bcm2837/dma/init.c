...
volatile struct dmaEngineRegister_t *dmaEngineRegs[BCM2837_DMA_CTRLBLCK_NUMBR] = {NULL};

static void bcm2837_dma_init(void)
{
    for (u8_t i = 0x0; i < BCM2837_DMA_CTRLBLCK_NUMBR; i++)
        dmaEngineRegs[i] = ADD_TO_PTR(BCM2837_DMA_ENTRY, i * BCM2837_DMA_CTRLBLCK_PITCH);
